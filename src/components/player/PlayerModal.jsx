...
function buildVideoState(v) {
  try {
    ...
    return { ...v, views: Number(v.views_count ?? v.views ?? 0) };
  }
}
      ...
      <div style={{ position: "relative", width: 40, height: 40 }}>
        <svg width={40} height={40} style={{ transform: "rotate(-90deg)" }}>
          ...
          <circle
            ...
            strokeWidth={3}
            strokeLinecap="round"
            strokeDasharray={`${(seconds / 5) * 100.53} 100.53`}
      ...
      >
        ...
        <button
          ...
          style={{
            ...
            fontSize: isMobile ? 11 : 13,
            background: canSkip ? "#fff" : "rgba(255,255,255,.25)",
            color: "#0a0a0a",
            border: "none",
            borderRadius: 8,
            fontWeight: 800,
...
export default function PlayerModal({ video: initVideo, onClose }) {
  ...
  // FIX: Track if we've already attempted autoplay for this video load
  ...
  useEffect(() => {
    let i = 0;
    const t = setInterval(() => {
      i = (i + 1) % SIDEBAR_ADS.length;
      setSidebarAd(SIDEBAR_ADS[i]);
    }, 30000);
  ...
  // View count increment after 5s of actual play
  useEffect(() => {
  ...
  // Sync view count updates from other contexts
  useEffect(() => {
    const handler = (e) => {
      if (e.detail.videoId === video.id) {
        ...
        if (cached) updatedData = { ...JSON.parse(cached), views: newViews };
  ...
  // ── FIX: Video src + event listeners — fast autoplay ──────────────────────
  // Key improvements:
  // 1. Set src directly on video element & call load() immediately
  // 2. Try to play as early as possible (on loadedmetadata or canplay, whichever comes first)
  // 3. Don't wait for canplaythrough — that is too late and causes long loading
  useEffect(() => {
    const v = vRef.current;
    if (!v) return;

    // Only reset + reload when the video actually changes
    if (currentVideoId.current !== video.id) {
      ...
      // Set src and preload eagerly
      v.src = video.video_url;
      v.preload = "auto";
      v.load();
    }

    // ── FIX: Attempt play as soon as we have enough metadata/data ──
    const tryPlay = () => {
      if (adActive) return; // Don't autoplay if ad is running
      ...
      v.play()
        ...
        .catch((err) => {
          // Autoplay blocked — show play button, let user tap
    ...
    const handleLoadedMetadata = () => {
      setDur(v.duration || 0);
      // Try to play immediately on metadata load (very early — usually < 1s)
      tryPlay();
    };

    const handleCanPlay = () => {
      setIsBuffering(false);
      tryPlay(); // Also try here in case loadedmetadata didn't fire first
    };

    const handleWaiting = () => {
      // Only show buffering if video was actually playing
      if (!v.paused) setIsBuffering(true);
    };
    ...
    const handleTimeUpdate = () => {
      ...
      setCurTime(current);
      setDur(duration);
      setProg(duration ? (current / duration) * 100 : 0);
    ...
    // If video is already in a ready state (e.g. cached), try immediately
    if (v.readyState >= 2) {
      setIsBuffering(false);
      tryPlay();
    }
  ...
  // ── FIX: seekBy — positive = forward, negative = backward ─────────────────
  // This was already correct in the original code but double-tap handler
  // had left/right zones swapped. Fixed below in handleTouchEnd.
  const seekBy = useCallback(
    (secs) => {
      ...
      // FIX: positive secs = forward = "fwd", negative secs = backward = "bwd"
  ...
  const enterFS = useCallback(() => {
    ...
    if (v?.webkitRequestFullScreen) {
      try { v.webkitRequestFullScreen(); return; } catch (e) {}
    }
    const req =
  ...
  const handleTouchEnd = useCallback(
    (e) => {
      ...
      // ── FIX: Left zone = backward, Right zone = forward ──
      // Left 35% of the player = seek backward
      // Right 35% of the player = seek forward
      const isLeftZone = x < containerWidth * 0.35;
      const isRightZone = x > containerWidth * 0.65;

      const dx = Math.abs(touch.clientX - touchStartRef.current.x);
      const dy = Math.abs(touch.clientY - touchStartRef.current.y);
      if (dx > 25 || dy > 25) return; // Ignore swipes
      ...
      if (gap < 300 && prevTap.time > 0) {
        clearTimeout(tapTimerRef.current);
        lastTap.current = { time: 0, x: 0 };

        if (isRightZone) {
          // ── FIX: Right zone double tap = FORWARD (+10s) ──
          seekBy(10);
          return;
        } else if (isLeftZone) {
          // ── FIX: Left zone double tap = BACKWARD (-10s) ──
          seekBy(-10);
          return;
        }
        // Double tap in center = toggle play
        togglePlay();
        return;
      }
  ...
  const controlProps = {
    ...
    vRef,
    captionLang,
    onCaptionChange: setCaptionLang,
    buffered,
    isBuffering,
    captionStatus: "off",
    togglePlay,
    ...
    onVolume: (n) => {
      setVol(n);
      if (vRef.current) {
        vRef.current.volume = n;
        vRef.current.muted = n === 0;
        setMuted(n === 0);
      }
    },
  };
        ...
        <div style={{ minWidth: 0 }}>
          ...
          <div
            ...
            style={{
              ...
              aspectRatio: isFS ? "unset" : "16/9",
              userSelect: "none",
              ...(isFS ? { position: "fixed", inset: 0, zIndex: 99999 } : {}),
            }}
          >
            {/* FIX: video element — preload="auto" set here too for markup clarity */}
            <video
              ref={vRef}
              playsInline
              preload="auto"
            ...
            {related.length > displayLimit && (
              <button
                ...
                onMouseLeave={(e) => {
                  ...
                  e.currentTarget.style.boxShadow = "0 4px 15px rgba(0,0,0,.3)";
                }}
              >
                Show More Videos
              </button>
  ...
  return (
    <button
      ...
      style={{
        ...
        padding: "8px 14px",
        borderRadius: 10,
        border: `1px solid ${active || hov ? color : C.border}`,
        background: active || hov ? color + "1a" : C.bg3,
  ...
  return (
    <>
      ...
      >
        <span>Up Next</span>
        {!isMobile && (
          <span style={{ fontSize: 10, color: C.muted, fontWeight: 500 }}>Press N for next</span>
        )}
      </div>
      <div
        style={{
      ...
      >
        {videos.map((v, idx) => (
          <RelatedCard key={v.id} video={v} index={idx} onPlay={onPlay} isMobile={isMobile} />
        ))}
      </div>
    </>
  );
  ...
  return (
    <div
      ...
      style={{
        ...
        gap: isMobile ? 0 : 10,
        padding: isMobile ? 0 : "8px 6px",
        borderRadius: 10,
        cursor: "pointer",
        background: hov ? C.bg3 : "transparent",
        transition: "background .2s",
        borderBottom: isMobile ? "none" : `1px solid ${C.border}22`,
      }}
    >
      <div
        style={{
      ...
      >
        <img
          src={
            v.thumbnail_url ||
            `https://picsum.photos/640/360?random=${String(v.id).charCodeAt?.(0) || index + 1}`
          }
          alt={v.title}
          loading="lazy"
          style={{
            ...
            height: "100%",
            objectFit: "cover",
            borderRadius: isMobile ? "10px 10px 0 0" : 8,
          }}
        />
        {v.is_vip && (
      ...
      <div style={{ flex: 1, minWidth: 0, padding: isMobile ? "7px 8px 8px" : 0 }}>
        ...
        <div style={{ fontSize: 10, color: C.accent, fontWeight: 600, marginBottom: 2 }}>
          {v.profiles?.display_name || v.channel || "Unknown"}
        </div>

...
const handleProfileClick = useCallback((e) => {
  ...
  // 2. Change the state FIRST (this triggers the App.js useEffect)
  if (profileData) setActiveProfile(profileData);
  setTab(`profile:${identifier}`);

  // 3. Close the modal SECOND 
  // This ensures the Modal doesn't "block" the main window from scrolling
  onClose();

  // 4. Force an immediate scroll
  window.scrollTo(0, 0);
  ...
  const enterFS = useCallback(() => {
    ...
    // 1. iOS iPhone Specific: Must be called on the VIDEO element
    if (v.webkitEnterFullscreen) {
      try {
        v.webkitEnterFullscreen();
        return;
      } catch (e) {
        console.error("iOS Fullscreen failed", e);
      }
    }

    // 2. Standard API (Android / iPad / Desktop)
    try {
      ...
      if (req) {
        req.call(el).catch(err => {
          // If element FS fails, try video FS as fallback
          v.requestFullscreen?.();
        });
      }
    } catch (e) {
      v.play(); // Fallback to ensure video keeps playing
    }
  }, []);
  ...
  const controlProps = {
    playing, muted, vol, prog, dur, curTime, speed, isFS, isMobile, showCtrl, vRef, captionLang, onCaptionChange: setCaptionLang, buffered, isBuffering, captionStatus: "off", togglePlay, seekBy, toggleFS, onMenuToggle: handleMenuToggle,
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 14 }}>
              ...
              <div
                ...
                style={{
                  flexShrink: 0,
                  cursor: "pointer",
                  WebkitTapHighlightColor: "transparent" // Add this line
                }}
              >
                <Avatar profile={pf} size={isMobile ? 34 : 42} />
              </div>
              {/* Name + followers — profile nav */}
              <div style={{ flex: 1, minWidth: 0, cursor: "pointer", WebkitTapHighlightColor: "transparent" }}>

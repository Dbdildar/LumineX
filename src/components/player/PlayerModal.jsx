...
const getStrategy=(count,videoId="")=>{const s=AD_STRATEGY[count];if(s)return{pre:s.pre||0,post:s.post||0,label:s.label||"Standard"};const isBumper=count>20&&(count%3===0||videoId.length%2===0);return isBumper?{pre:6,post:0,label:"Power Bumper"}:{pre:0,post:0,label:"Organic"};};
const UNSKIPPABLE=new Set([5,6,10,30]);
const DUMMY_ADS=[
  {id:1,brand:"NovaDrive",tagline:"The Future, Delivered.",cta:"Learn More →",description:"Experience the all-new NovaDrive EV.",badge:"⚡ Electric",bg:"linear-gradient(135deg,#0f0c29,#302b63,#24243e)",accent:"#7c6bfa",logo:"🚗",category:"Automotive",url:"#"},
  ...
  {id:3,brand:"ArcFit Pro",tagline:"Train Smarter. Live Longer.",cta:"Get Free Trial →",description:"AI-powered workout plans that adapt to YOU.",badge:"💪 Health",bg:"linear-gradient(135deg,#001a12,#00412c,#001a12)",accent:"#00e676",logo:"🏋️",category:"Health",url:"#"},
...
function buildVideoState(v){try{const cached=sessionStorage.getItem(`video_${v.id}`);const initViews=Number(v.views_count??v.views??0);if(cached){const p=JSON.parse(cached);return{...v,...p,views:Math.max(Number(p.views_count??p.views??0),initViews)};}return{...v,views:initViews};}catch{return{...v,views:Number(v.views_count??v.views??0)};}}
const truncateTitle=(str="",max=60)=>str.length>max?str.slice(0,max).trimEnd()+"…":str;
...
function SeekFlash({seekFlash,arcProg}){
  ...
  const Panel=({side,active,icon,label})=>(
    <div style={{position:"absolute",[side]:0,top:0,bottom:0,width:"38%",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",background:active?`radial-gradient(ellipse at ${side==="left"?"30%":"70%"} 50%, rgba(255,255,255,.18) 0%, transparent 70%)`:"transparent",transition:"background .15s",pointerEvents:"none"}}>
      {active&&(<div style={{position:"relative",width:76,height:76}}>
        <svg width={76} height={76} style={{position:"absolute",inset:0,transform:"rotate(-90deg)"}}>
          <circle cx={38} cy={38} r={30} fill="none" stroke="rgba(255,255,255,.15)" strokeWidth={3}/>
          <circle cx={38} cy={38} r={30} fill="none" stroke="white" strokeWidth={3} strokeLinecap="round" strokeDasharray={`${arc} ${C2}`} style={{transition:"stroke-dasharray .15s"}}/>
        </svg>
  ...
  return(
    <div style={{position:"absolute",inset:0,zIndex:12,pointerEvents:"none"}}>
      ...
      {seekFlash==="3x"&&(<div style={{position:"absolute",inset:0,display:"flex",alignItems:"center",justifyContent:"center",background:"rgba(0,0,0,.25)"}}>
        <div style={{background:"rgba(0,0,0,.7)",border:"1px solid rgba(255,255,255,.3)",borderRadius:12,padding:"8px 18px",fontSize:16,fontWeight:900,color:"white",backdropFilter:"blur(8px)"}}>3× Speed</div>
      </div>)}
    </div>
  );
}
...
function BufferingOverlay({show}){
  ...
  useEffect(()=>{if(!show){setVisible(false);return;}const t=setTimeout(()=>setVisible(true),400);return()=>clearTimeout(t);},[show]);
  if(!visible)return null;
  return(
    <div style={{position:"absolute",inset:0,zIndex:15,pointerEvents:"none",display:"flex",alignItems:"center",justifyContent:"center",background:"rgba(0,0,0,.35)",backdropFilter:"blur(2px)"}}>
      <style>{`@keyframes lxSpinRing{to{stroke-dashoffset:-251.2;}}@keyframes lxPulseGlow{0%,100%{opacity:.6;transform:scale(1)}50%{opacity:1;transform:scale(1.1)}}`}</style>
      <div style={{position:"relative",width:72,height:72,display:"flex",alignItems:"center",justifyContent:"center"}}>
        <svg style={{position:"absolute",inset:0}} width={72} height={72} viewBox="0 0 72 72"><circle cx={36} cy={36} r={32} fill="none" stroke="rgba(255,255,255,.08)" strokeWidth={4}/></svg>
        <svg style={{position:"absolute",inset:0,transform:"rotate(-90deg)"}} width={72} height={72} viewBox="0 0 72 72">
          <circle cx={36} cy={36} r={32} fill="none" stroke="url(#lxGrad)" strokeWidth={4} strokeLinecap="round" strokeDasharray="60 140" style={{animation:"lxSpinRing .9s linear infinite",transformOrigin:"36px 36px"}}/>
          <defs><linearGradient id="lxGrad" x1="0%" y1="0%" x2="100%" y2="0%"><stop offset="0%" stopColor="var(--accent)"/><stop offset="100%" stopColor="var(--accent2,#818cf8)"/></linearGradient></defs>
        </svg>
        <div style={{width:14,height:14,borderRadius:"50%",background:"linear-gradient(135deg,var(--accent),var(--accent2,#818cf8))",boxShadow:"0 0 20px var(--accent)",animation:"lxPulseGlow 1.2s ease-in-out infinite"}}/>
      </div>
    </div>
  );
}
...
function AutoPlayCountdown({seconds,onPlay,onCancel}){
  return(
    <div style={{position:"absolute",bottom:80,right:16,zIndex:30,background:"rgba(0,0,0,.88)",backdropFilter:"blur(12px)",border:`1px solid ${C.accent}44`,borderRadius:14,padding:"12px 16px",display:"flex",alignItems:"center",gap:12,animation:"fadeIn .3s ease"}}>
      <div style={{position:"relative",width:40,height:40}}>
        <svg width={40} height={40} style={{transform:"rotate(-90deg)"}}><circle cx={20} cy={20} r={16} fill="none" stroke={C.border} strokeWidth={3}/><circle cx={20} cy={20} r={16} fill="none" stroke={C.accent} strokeWidth={3} strokeLinecap="round" strokeDasharray={`${(seconds/5)*100.53} 100.53`} style={{transition:"stroke-dasharray 1s linear"}}/></svg>
      ...
      <div><div style={{fontSize:10,color:C.muted,marginBottom:2}}>Up Next</div><div style={{fontSize:12,fontWeight:700,color:C.text}}>Auto-playing…</div></div>
    ...
    <div style={{position:"absolute",inset:0,zIndex:100,background:adData.bg,display:"flex",flexDirection:"column",overflow:"hidden",animation:"fadeIn .25s ease"}}>
      ...
      <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",padding:isMobile?"10px 12px":"14px 20px",borderBottom:`1px solid ${adData.accent}22`,position:"relative",zIndex:2}}>
        <div style={{display:"flex",alignItems:"center",gap:10}}>
          ...
          <div><div style={{fontSize:isMobile?11:14,fontWeight:800,color:"#fff"}}>{adData.brand}</div>{!isMobile&&<div style={{fontSize:9,color:adData.accent,fontWeight:700,letterSpacing:1,textTransform:"uppercase"}}>{adData.category}</div>}</div>
        </div>
      ...
      <div style={{display:"flex",alignItems:"center",padding:isMobile?"10px 16px":"14px 20px",borderTop:`1px solid ${adData.accent}22`,position:"relative",zIndex:2,gap:12,background:isMobile?"rgba(0,0,0,.4)":"transparent"}}>
        <div style={{position:"relative",width:40,height:40,flexShrink:0}}>
          <svg width={40} height={40} style={{transform:"rotate(-90deg)"}}><circle cx={20} cy={20} r={18} fill="none" stroke="rgba(255,255,255,.1)" strokeWidth={3}/><circle cx={20} cy={20} r={18} fill="none" stroke={adData.accent} strokeWidth={3} strokeLinecap="round" strokeDasharray={`${2*Math.PI*18*(1-progress)} ${2*Math.PI*18}`} style={{transition:"stroke-dasharray .3s linear"}}/></svg>
        ...
        <button disabled={!canSkip} onClick={onSkip} style={{padding:isMobile?"7px 16px":"9px 22px",fontSize:isMobile?11:13,background:canSkip?"#fff":"rgba(255,255,255,.25)",color:"#0a0a0a",border:"none",borderRadius:8,fontWeight:800,cursor:canSkip?"pointer":"default",transition:"all .2s",whiteSpace:"nowrap",fontFamily:"inherit"}}>
    ...
    <div style={{borderRadius:14,overflow:"hidden",border:`1px solid ${ad.accent}33`,marginBottom:20,animation:"fadeIn .4s ease",position:"relative"}}>
      ...
      <div style={{background:ad.bg,padding:"18px 16px 14px",display:"flex",flexDirection:"column",gap:8}}>
        <div style={{display:"flex",alignItems:"center",gap:10}}>
          ...
          <div><div style={{fontSize:13,fontWeight:800,color:"#fff"}}>{ad.brand}</div><div style={{fontSize:10,color:ad.accent,fontWeight:600}}>{ad.tagline}</div></div>
        </div>
...
export default function PlayerModal({video:initVideo,onClose}){
  ...
  const wrapRef=useRef(null),vRef=useRef(null),ctrlTimer=useRef(null),flashTimer=useRef(null);
  const autoTimer=useRef(null),adTimerRef=useRef(null),lpTimerRef=useRef(null),tapTimerRef=useRef(null);
  const lastTap=useRef({time:0,x:0}),viewGuard=useRef(false),currentVideoId=useRef(null),autoPlayAttempted=useRef(false);
  ...
  useEffect(()=>{let i=0;const t=setInterval(()=>{i=(i+1)%SIDEBAR_ADS.length;setSidebarAd(SIDEBAR_ADS[i]);},30000);return()=>clearInterval(t);},[]);

  // FIX: Profile click — only from explicit avatar/username tap
  const handleProfileClick=useCallback((e)=>{
    if(e){e.stopPropagation();e.preventDefault();}
    const profileData=video?.profiles,identifier=profileData?.username||video?.channel;
    if(!identifier)return;
    if(vRef.current)vRef.current.pause();
    window.dispatchEvent(new CustomEvent("lx_pause_video"));
    onClose();window.scrollTo(0,0);
    setTimeout(()=>{if(profileData)setActiveProfile(profileData);setTab(`profile:${identifier}`);},50);
  },[video,onClose,setActiveProfile,setTab]);

  useEffect(()=>{const h=()=>{if(vRef.current)vRef.current.pause();};window.addEventListener("lx_pause_video",h);return()=>window.removeEventListener("lx_pause_video",h);},[]);
  useEffect(()=>{viewGuard.current=false;setVideo(buildVideoState(initVideo));},[initVideo.id]); // eslint-disable-line

  const startAd=useCallback((duration,type,idx)=>{
    if(duration<=0)return;
    if(adTimerRef.current)clearInterval(adTimerRef.current);
    const unskippable=UNSKIPPABLE.has(duration),wait=unskippable?duration:5;
    setAdTotalDur(duration);setAdActive(true);setAdTime(duration);setCanSkip(false);setAdPart(type);
    setAdData(idx!==undefined?DUMMY_ADS[idx%DUMMY_ADS.length]:getAdForSession(1));
    setPlaying(false);if(vRef.current)vRef.current.pause();
    let elapsed=0;
    adTimerRef.current=setInterval(()=>{
      elapsed++;if(!unskippable&&elapsed>=wait)setCanSkip(true);
      setAdTime(prev=>{const n=prev-1;if(n<=0){clearInterval(adTimerRef.current);adTimerRef.current=null;if(unskippable)setCanSkip(true);return 0;}return n;});
    },1000);
  },[]);
  ...
  useEffect(()=>{
    const timer=setTimeout(async()=>{
      if(video?.id&&!viewGuard.current&&playing&&!adActive){
        ...
        try{await incrementView(video.id);}
        catch(err){console.error("Failed to increment view:",err);viewGuard.current=false;}
      }
  ...
  useEffect(()=>{
    const handler=(e)=>{
      if(e.detail.videoId===video.id){
        const newViews=e.detail.views;setVideo(prev=>({...prev,views:newViews}));
        const cached=sessionStorage.getItem(`video_${video.id}`);
        sessionStorage.setItem(`video_${video.id}`,JSON.stringify(cached?{...JSON.parse(cached),views:newViews}:{views:newViews}));
      }
    };
    window.addEventListener("video_view_updated",handler);return()=>window.removeEventListener("video_view_updated",handler);
  },[video.id]);

  const executeDownload=useCallback(()=>{
    const a=document.createElement("a");a.href=video.video_url;a.download=(video.title||"video")+".mp4";a.target="_blank";
    document.body.appendChild(a);a.click();document.body.removeChild(a);showToast("Download started!","success");
  },[video,showToast]);
  ...
  const skipAd=useCallback(()=>{
    ...
    if(adPart==="pre"){const v=vRef.current;if(v)v.play().then(()=>setPlaying(true)).catch(()=>{});}
    else if(adPart==="post"){if(dlPending){executeDownload();setDlPending(false);}else{const next=related[0];if(next){setVideo(buildVideoState(next));setProg(0);setCurTime(0);setDur(0);}}}
  },[adPart,dlPending,executeDownload,related]);

  useEffect(()=>{if(adActive&&canSkip&&adTime===0){const t=setTimeout(()=>{if(adActive&&canSkip)skipAd();},500);return()=>clearTimeout(t);}},[adActive,canSkip,adTime,skipAd]);
  ...
  const playNext=useCallback(()=>{cancelAuto();const next=related[0];if(next){setVideo(buildVideoState(next));setProg(0);setCurTime(0);setDur(0);}},[related,cancelAuto]);
  const startAutoCountdown=useCallback(()=>{
    if(!related.length)return;setAutoCountdown(5);let count=5;
    autoTimer.current=setInterval(()=>{count--;setAutoCountdown(count);if(count<=0){clearInterval(autoTimer.current);setAutoCountdown(null);const n=related[0];if(n){setVideo(buildVideoState(n));setProg(0);setCurTime(0);setDur(0);}}}
    ,1000);
  },[related]);

  useEffect(()=>{
    setDisplayLimit(4);setRelated(DEMO_VIDEOS.filter(v=>v.id!==video.id));
    videoAPI.getFeed({limit:100}).then(data=>{if(data?.length)setRelated(data.filter(v=>v.id!==video.id));}).catch(()=>{});
  },[video.id]);

  // ── Fast autoplay effect ────────────────────────────────────────────────────
  useEffect(()=>{
    const v=vRef.current;if(!v)return;
    if(currentVideoId.current!==video.id){
      currentVideoId.current=video.id;autoPlayAttempted.current=false;
      setProg(0);setCurTime(0);setDur(0);setBuffered(0);setIsBuffering(false);
      v.src=video.video_url;v.preload="auto";v.load();
    }
    const tryPlay=()=>{
      if(adActive||autoPlayAttempted.current)return;
      autoPlayAttempted.current=true;
      v.play().then(()=>{setPlaying(true);setIsBuffering(false);}).catch(err=>{console.warn("Autoplay blocked:",err.message);setPlaying(false);});
    };
    const onMeta=()=>{setDur(v.duration||0);tryPlay();};
    const onCanPlay=()=>{setIsBuffering(false);tryPlay();};
    const onWaiting=()=>{if(!v.paused)setIsBuffering(true);};
    const onPlaying=()=>{setIsBuffering(false);setPlaying(true);};
    const onPause=()=>setPlaying(false);
    const onTime=()=>{
      const cur=v.currentTime,d=v.duration||0;setCurTime(cur);setDur(d);setProg(d?(cur/d)*100:0);
      if(v.buffered.length>0){const bEnd=v.buffered.end(v.buffered.length-1);setBuffered(d?(bEnd/d)*100:0);}
    };
    const onEnded=()=>{setPlaying(false);const strat=getStrategy(sessionCount-1,video.id);if(strat.post>0)startAd(strat.post,"post",sessionCount);else startAutoCountdown();};
    const onErr=()=>{setIsBuffering(false);setPlaying(false);};
    v.addEventListener("loadedmetadata",onMeta);v.addEventListener("canplay",onCanPlay);v.addEventListener("waiting",onWaiting);
    v.addEventListener("playing",onPlaying);v.addEventListener("pause",onPause);v.addEventListener("timeupdate",onTime);
    v.addEventListener("ended",onEnded);v.addEventListener("error",onErr);
    if(v.readyState>=2){setIsBuffering(false);tryPlay();}
  ...
  },[video.id,video.video_url,adActive,sessionCount,startAd,startAutoCountdown]);
  ...
  useEffect(()=>{
    const v=vRef.current;if(!v)return;
    const onBegin=()=>setIsFS(true),onEnd=()=>setIsFS(false);
    v.addEventListener("webkitbeginfullscreen",onBegin);v.addEventListener("webkitendfullscreen",onEnd);
    return()=>{v.removeEventListener("webkitbeginfullscreen",onBegin);v.removeEventListener("webkitendfullscreen",onEnd);};
  },[]);

  const revealCtrl=useCallback(()=>{setShowCtrl(true);clearTimeout(ctrlTimer.current);if(!isMenuOpen)ctrlTimer.current=setTimeout(()=>setShowCtrl(false),3500);},[isMenuOpen]);
  const handleMenuToggle=useCallback((isOpen)=>{setIsMenuOpen(isOpen);if(isOpen){clearTimeout(ctrlTimer.current);setShowCtrl(true);}else ctrlTimer.current=setTimeout(()=>setShowCtrl(false),4000);},[]);
  const togglePlay=useCallback(()=>{const v=vRef.current;if(!v||adActive)return;if(v.paused){v.play().then(()=>setPlaying(true)).catch(()=>{});revealCtrl();}else{v.pause();setPlaying(false);setShowCtrl(true);clearTimeout(ctrlTimer.current);}},[revealCtrl,adActive]);
  ...
  // ── FIX: Fullscreen — iOS Chrome/Safari require webkitEnterFullscreen on <video> ──
  const enterFS=useCallback(()=>{
    const el=wrapRef.current,v=vRef.current;
    try{if(window.screen?.orientation?.lock)window.screen.orientation.lock("landscape").catch(()=>{});}catch(_){}
    // iOS: MUST use webkitEnterFullscreen on the video element
    if(v){
      if(typeof v.webkitEnterFullscreen==="function"){try{v.webkitEnterFullscreen();return;}catch(_){}}
      if(typeof v.webkitRequestFullScreen==="function"){try{v.webkitRequestFullScreen();return;}catch(_){}}
    }
    // Standard API for Android + desktop
    if(el){
      const req=el.requestFullscreen||el.mozRequestFullScreen||el.webkitRequestFullscreen||el.msRequestFullscreen;
      if(req){req.call(el).catch(()=>{});return;}
    }
    // Absolute fallback on video el
    if(v){const req=v.requestFullscreen||v.mozRequestFullScreen||v.webkitRequestFullscreen||v.msRequestFullscreen;if(req)req.call(v).catch(()=>{});}
  },[]);

  const exitFS=useCallback(()=>{
    try{if(window.screen?.orientation?.unlock)window.screen.orientation.unlock();}catch(_){}
  ...
  useEffect(()=>{
    const h=(e)=>{
      ...
      if(e.key==="ArrowRight")seekBy(10);if(e.key==="ArrowLeft")seekBy(-10);
      if(e.key==="f"||e.key==="F")toggleFS();if(e.key==="n"||e.key==="N")playNext();
  ...
  const handleTouchStart=useCallback((e)=>{
    if(adActive)return;
    const touch=e.touches[0];touchStartRef.current={x:touch.clientX,y:touch.clientY,t:Date.now()};
    clearTimeout(lpTimerRef.current);
    lpTimerRef.current=setTimeout(()=>{const v=vRef.current;if(!v||v.paused)return;v.playbackRate=3;setSpeed(3);setIs3x(true);setSeekFlash("3x");clearTimeout(flashTimer.current);},600);
  },[adActive]);

  const handleTouchEnd=useCallback((e)=>{
    clearTimeout(lpTimerRef.current);
    if(is3x){const v=vRef.current;if(v){v.playbackRate=1;setSpeed(1);}setIs3x(false);setSeekFlash(null);setArcProg(0);return;}
    if(adActive)return;
    const rect=e.currentTarget.getBoundingClientRect(),touch=e.changedTouches[0];
    const x=touch.clientX-rect.left,w=rect.width;
    const isLeft=x<w*0.35,isRight=x>w*0.65;
    const dx=Math.abs(touch.clientX-touchStartRef.current.x),dy=Math.abs(touch.clientY-touchStartRef.current.y);
    if(dx>25||dy>25)return;
    const now=Date.now(),prev=lastTap.current,gap=now-prev.time;
    if(gap<300&&prev.time>0){
      clearTimeout(tapTimerRef.current);lastTap.current={time:0,x:0};
      if(isRight){seekBy(10);return;}else if(isLeft){seekBy(-10);return;}togglePlay();return;
    }
    lastTap.current={time:now,x};clearTimeout(tapTimerRef.current);
    tapTimerRef.current=setTimeout(()=>{
      if(!isLeft&&!isRight)togglePlay();else{if(showCtrl)setShowCtrl(false);else revealCtrl();}
      lastTap.current={time:0,x:0};
    },280);
  },[adActive,is3x,seekBy,togglePlay,showCtrl,revealCtrl]);

  const handleShare=async()=>{const url=`${window.location.origin}?v=${video.id}`;try{if(navigator.share){await navigator.share({title:video.title,url});showToast("Thanks for sharing!","success");}else{await navigator.clipboard.writeText(url);showToast("Link copied! 📋","success");}}catch(e){if(e.name!=="AbortError")showToast("Could not share","error");}};
  const handleDownload=()=>{if(video.is_vip&&profile?.is_vip){executeDownload();return;}showToast("Ad starting… download begins after 30s 📥","info");setDlPending(true);startAd(30,"post",sessionCount);};
  const handleSave=async()=>{if(!session){setAuthModal("login");return;}const next=!saved;setSaved(next);await likeAPI.toggleSave(session.user.id,video.id,saved);showToast(next?"❤️ Saved!":"Removed from saved","success");};
  const playRelated=useCallback((v)=>{cancelAuto();setVideo(buildVideoState(v));setProg(0);setCurTime(0);setDur(0);wrapRef.current?.scrollIntoView({behavior:"smooth",block:"start"});},[cancelAuto]);

  const controlProps={playing,muted,vol,prog,dur,curTime,speed,isFS,isMobile,showCtrl,vRef,captionLang,onCaptionChange:setCaptionLang,buffered,isBuffering,captionStatus:"off",togglePlay,seekBy,toggleFS,onMenuToggle:handleMenuToggle,
    ...
    <div style={{position:"fixed",inset:0,zIndex:9999,background:C.bg,overflowY:"auto",overflowX:"hidden",animation:"fadeIn .15s ease"}}>
      ...
      <div style={{position:"sticky",top:0,zIndex:100,background:"var(--headerBg)",backdropFilter:"blur(20px)",borderBottom:`1px solid ${C.border}`,display:"flex",alignItems:"center",gap:12,padding:isMobile?"10px 12px":"10px 20px",height:52}}>
        ...
        <div style={{flex:1,minWidth:0}}>
          <div style={{fontSize:13,fontWeight:700,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap",color:C.text}}>{truncateTitle(video.title,isMobile?30:60)}</div>
          <div style={{fontSize:11,color:C.muted}}>{pf.display_name||pf.username||""}</div>
        </div>
        {isFS&&<button onClick={exitFS} style={{background:"rgba(0,0,0,.6)",border:`1px solid ${C.border}`,borderRadius:8,padding:"5px 12px",color:"white",fontSize:12,fontWeight:700,cursor:"pointer",fontFamily:"inherit",flexShrink:0}}>⊡ Exit</button>}
      </div>
      ...
      <div style={{display:"grid",gridTemplateColumns:isMobile?"1fr":"minmax(0,1fr) 340px",maxWidth:"100%",paddingBottom:isMobile?80:0}}>
        <div style={{minWidth:0}}>
          ...
          <div ref={wrapRef} onMouseMove={revealCtrl}
            onMouseLeave={()=>{if(!isMobile&&playing&&!isMenuOpen){clearTimeout(ctrlTimer.current);ctrlTimer.current=setTimeout(()=>setShowCtrl(false),600);}}}
            style={{position:"relative",background:"#000",width:"100%",overflow:isMobile?"visible":"hidden",aspectRatio:isFS?"unset":"16/9",userSelect:"none",...(isFS?{position:"fixed",inset:0,zIndex:99999}:{})}}>
            <video ref={vRef} playsInline preload="auto"
            ...
            {isFS&&!adActive&&<button onClick={exitFS} style={{position:"absolute",top:14,right:14,zIndex:40,background:"rgba(0,0,0,.6)",border:"1px solid rgba(255,255,255,.2)",borderRadius:8,padding:"6px 14px",color:"white",fontSize:12,fontWeight:700,cursor:"pointer",fontFamily:"inherit",backdropFilter:"blur(8px)"}}>⊡ Exit</button>}
            <ControlsBar {...controlProps}/>
          </div>

          {/* Video info */}
          <div style={{padding:isMobile?"14px 12px":"18px 20px"}}>
            <h1 style={{fontSize:isMobile?15:20,fontWeight:800,lineHeight:1.35,marginBottom:12,fontFamily:"'Syne',sans-serif",color:C.text}}>
              {truncateTitle(video.title,isMobile?70:120)}
            </h1>

            {/* ── Channel row: avatar+username = profile link. Stats = NOT a link ── */}
            <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:14}}>
              {/* Avatar — profile nav */}
              <div onClick={handleProfileClick} style={{flexShrink:0,cursor:"pointer"}}>
                <Avatar profile={pf} size={isMobile?34:42}/>
              </div>
              {/* Name + followers — profile nav */}
              <div onClick={handleProfileClick} style={{flex:1,minWidth:0,cursor:"pointer"}}>
                <div style={{display:"flex",alignItems:"center",gap:5}}>
                  <span style={{fontSize:13,fontWeight:700,color:C.text,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap",maxWidth:isMobile?120:200}}>
              ...
              {/* Stats — NO profile nav, just info */}
            ...
            {isMobile&&(
              <div style={{marginBottom:24}}>
                ...
                {related.length>displayLimit&&<button onClick={()=>setDisplayLimit(p=>p+20)} style={loadMoreStyle}>Show More Videos</button>}
              </div>
            )}
...
function RelatedCard({video:v,index,onPlay,isMobile}){
  const[hov,setHov]=useState(false);
  return(<div onClick={()=>onPlay(v)} onMouseEnter={()=>setHov(true)} onMouseLeave={()=>setHov(false)}
...
const loadMoreStyle={width:"100%",padding:"12px 0",marginTop:16,background:"linear-gradient(135deg,var(--accent),var(--accent2,#818cf8))",border:"none",borderRadius:12,color:"white",fontSize:13,fontWeight:800,letterSpacing:.8,textTransform:"uppercase",cursor:"pointer",boxShadow:"0 4px 15px rgba(0,0,0,.3)",transition:"all .3s ease",fontFamily:"inherit",display:"flex",alignItems:"center",justifyContent:"center"};

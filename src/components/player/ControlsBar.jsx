...
function CaptionPicker({ selectedLang, onSelect, videoEl, captionStatus, isMobile, onMenuToggle }) {
  ...
  useEffect(() => {
    ...
    const h = (e) => {
      if (ref.current && !ref.current.contains(e.target)) handleOpen(false);
    };
  ...
  const handleSelect = (lang) => {
    ...
    handleOpen(false);
    if (videoEl) {
      const t = videoEl.textTracks;
      for (let i = 0; i < t.length; i++) if (t[i].label !== "AI Caption") t[i].mode = "hidden";
    }
  };
  ...
  const triggerBtn = (
    <button
      ...
      style={{
        ...
        borderRadius: 6,
        color: isOn ? C.accent : "rgba(255,255,255,.85)",
        fontSize: 11,
        fontWeight: 800,
        cursor: "pointer",
        padding: "5px 9px",
  ...
  const langList = CAPTION_LANGS.map((lang) => (
    <div
      ...
      style={{
        ...
        alignItems: "center",
        gap: 12,
        color: selectedLang === lang.code ? C.accent : "black",
  ...
  return (
    ...
    >
      ...
      >
        ...
        {/* FIX: seekBy(-10) = backward, seekBy(10) = forward — icons match direction */}
